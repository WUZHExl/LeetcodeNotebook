 ...
 */
 var threeSum = function(nums) {
     ...
     for(let i=0;i<nums.length;i++){
      ...
      if(nums[i]>0) break;
      if(i>0&&nums[i]===nums[i-1]) continue //去重//因为排好了序，所以相邻的元素必然相邻

      while(l<r){
        if(nums[i]+nums[l]+nums[r]===0){
          ans.push([nums[i],nums[l],nums[r]])
          while(l<r&&nums[l]===nums[l+1]) l++ //去重//因为排好了序，所以相邻的元素必然相邻
          while(l<r&&nums[r]===nums[r-1]) r-- //去重//因为排好了序，所以相邻的元素必然相邻
          l++
          r--
        }
